...
import {displayDelaunay, displayVoronoi} from "./Visualize";
import {mat, vec, Vector} from "@josh-brown/vector";
...
const H = mat([0,3,6,9]
    ...
    .map(theta => [Math.cos(theta), Math.sin(theta)]))
    .transpose()
function W(x : Vector<number>) : boolean {
...
let points = model_set.computePoints(vec([0,0,0,0]))
